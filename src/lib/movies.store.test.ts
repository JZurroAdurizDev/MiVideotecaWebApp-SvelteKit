...
describe('Movies Store (Svelte 5 Runes)', () => {
  ...
  describe('toggleFavorite()', () => {
    it('debería actualizar el estado isFavorite de una película en el store', async () => {
      vi.mocked(api.getMovies).mockResolvedValue([...mockMovies]);
      await moviesStore.loadMovies();

      const favoritedMovie = { ...mockMovies[0], isFavorite: true };
    ...
    it('debería manejar error al actualizar favorito', async () => {
